...
public class RecoveryMgr {
        ...
        while (iter.hasNext()) {
            ...
            switch (rec.op()) {
                ...
                case NQCKPT:
                    ...
                    int[] txList = nqRec.txList();
                    for (int tx : txList) {
                        if (!finishedTxs.contains(tx)) {
                            earliestActiveTx = tx;
                            break;
                        }
                    }
                    break;
                case START:
                    if (earliestActiveTx == rec.txNumber())
                        return;
                    break;
                case COMMIT:
                case ROLLBACK:
    ...
    /**
     * A method to write a non-quiescent checkpoint record to the log.
     *
     * @param activeTxns the list of active transactions
     */
    public void checkpoint(int[] activeTxns) {
